...
def lipstick_md(present_us,present_eu,present_gb,present_cad):
    ...
    ws.conditional_format(us_row +eu_row +gb_row+ 14,4,us_row +eu_row + gb_row +cad_row+ 11,4, {'type': '2_color_scale','min_color':'#e3e7ea', 'max_color':'#cadbec'})
    ws.conditional_format(us_row +eu_row +gb_row+ 14,5,us_row +eu_row + gb_row+cad_row+ 11,5, {'type': '2_color_scale','min_color':'#e3e7ea', 'max_color':'#cadbec'})

    t_minus_days[0].value
    ws.insert_image('A1', 'Y:\\Analytics and Tools\\Swaps\\Scrape Data\\logo.png')
    ws.merge_range('D2:J2','Snapshot of Meeting Dates traded '+day(t_minus_days[0].value.day,t_minus_days[0].value.month,t_minus_days[0].value.year),merge_format)
    ws.write('D4','FOMC',f1)
    ws.write('E4','FF',f1)
    ...
    ws.write(12 + us_row + eu_row + gb_row,5,sonia,f1)
...
metgn = [f'USSOFED{i} Curncy' for i in range(1,9)]+[f'EESF{i}A Curncy' for i in range(1,9)]+[f'GPSF{i}A Curncy' for i in range(1,9)]+[f'CDSF{i}A Curncy' for i in range(1,9)]
...
def automate_execution():
    ...
    global g, initial_index, fed_funds,estron,sonia,mds, meeting_info
...
def run():    
    ...
    lipstick_md(present_us,present_eu,present_gb,present_cad)
