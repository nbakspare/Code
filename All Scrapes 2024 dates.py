...
fmc = pd.DataFrame({'Date': [ '09/18/2024','11/07/2024','12/18/2024']})#,'01/29/2025']})
ecb = pd.DataFrame({'Date': ['09/18/2024','10/23/2024','12/18/2024']})#,'10/23/2024','12/18/2024']})
mpc = pd.DataFrame({'Date': ['09/19/2024','11/07/2024','12/19/2024']})#,'11/07/2024','12/19/2024']})
cads = pd.DataFrame({'Date': ['09/05/2024','10/24/2024','12/12/2024']})
rba = pd.DataFrame({'Date': ['08/07/2024','09/25/2024','11/06/2024','12/11/2024','02/19/2025','04/02/2025','05/21/2025']})
rbnz = pd.DataFrame({'Date': ['08/15/2024','10/10/2024','11/28/2024','02/20/2025','04/10/2025','05/29/2025']})
...
fomc_adjust = IntText(description = 'FOMC Decision (bps)', style = style) # adjust for cb action
ecb_adjust = IntText(description = 'ECB Decision (bps)',style = style) # adjust for cb action
mpc_adjust = IntText(description = 'MPC Decision (bps)',style = style) # adjust for cb action
cad_adjust = IntText(description = 'CAD Decision (bps)',style = style) # adjust for cb action
au_adjust = IntText(description = 'AUD Decision (bps)',style = style) # adjust for cb action
nz_adjust = IntText(description = 'NZD Decision (bps)',style = style) # adjust for cb action


commit_dates = Button(description = 'Commit')

output1 = Output()

display(HBox([VBox(t_minus_days),VBox([fomc_adjust,ecb_adjust,mpc_adjust,cad_adjust,au_adjust,nz_adjust])]))
display(commit_dates,output1)
...
metgn = [f'USSOFED{i} Curncy' for i in range(1,9)]+[f'EESF{i}A Curncy' for i in range(1,9)]+[f'GPSF{i}A Curncy' for i in range(1,9)]
...
def on_button_clicked1(b):
    ...
    global g, initial_index, fed_funds,estron,sonia,mds, meeting_info, caddy, aussie, newz
    with output1:
        ...
        if t_minus_days[0].value.date()>fmc.loc[0,'Date'].date():
        ...
        elif t_minus_days[0].value.date()>cads.loc[0,'Date'].date():
            print(f'The CAD swaps have rolled. Please toggle the code on, and in the code cell below the INTRODUCTION\n please delete the date for the {cads.loc[0,"Date"].strftime("%B")} meeting. Please Run All cells after that ')
        elif t_minus_days[0].value.date()>rba.loc[0,'Date'].date():
            print(f'The AUD swaps have rolled. Please toggle the code on, and in the code cell below the INTRODUCTION\n please delete the date for the {rba.loc[0,"Date"].strftime("%B")} meeting. Please Run All cells after that ')
        elif t_minus_days[0].value.date()>rbnz.loc[0,'Date'].date():
            print(f'The RBNZ swaps have rolled. Please toggle the code on, and in the code cell below the INTRODUCTION\n please delete the date for the {rbnz.loc[0,"Date"].strftime("%B")} meeting. Please Run All cells after that ')
            ...
            display(pd.DataFrame([f"{i.value.strftime('%B')} {i.value.day}" for i in t_minus_days], index = [i.description.upper() for i in t_minus_days], columns = ['DATES']).T)

        
commit_dates.on_click(on_button_clicked1)    
...
scrape_run = Button(description = 'Run Scrapes')
output2 = Output()
display(scrape_run, output2)
