...
def generate_trade(df,args,weightthres,three_leg,pca,weight_overide):
    ...
    if three_leg == True: 
        ...
        adf_res = adfuller(df_EMA - df)#(df_EMA - df)
...
#                         spread = weights[0] * x[i+t:i+wind+t] - weights[1] * y[i+t:i+wind+t]
    ...
    #     trade_analytics(oup,instrument)
        ...
        return here, df_save, ema, flag_ibg, end_of_spread, end_of_exp, take_prof, total_num, win_ratio,avg_time, max_drawdown, cum_pnl, avg_pnl, max_pnl
    else:
...
def trade_stats(PnL_pc,PnL_abs,time_held,outcome,extype,args,instrument):
    ...
    if instrument == 'future':
        ...
        if total_num > 0:
            ...
            print('No Trades Completed')
    return win_ratio,avg_time, max_drawdown, cum_pnl, avg_pnl
